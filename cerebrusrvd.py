...
semantic vector search process by using LLMs like Cerebrus Llama-3.1-70B
to prioritize terms in the query and then matching for similar vectors
in the order of prioritized terms. 

The terms may be from the query or generated by the `Cerebrus Llama-3.1-70B` to
...
if st.button("Vector Search!"):
	if ta_value == "" or uq_value == "":
		...
		model = load_model()
		llm_response = cerebrus_rvd_api(user_query)
		...
		st.subheader("Cerebrus LLM Generated Priority Terms")
